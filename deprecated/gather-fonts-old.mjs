...
const CONCURRENT_DOWNLOADS = 1; // Reduced to avoid rate limiting
const MAX_RETRIES = 3;
...
async function processFont(family, cache) {
  ...
  if (cache[cacheKey]) {
    ...
    console.log(`Using cached variants for ${family}`);
  } else {
      ...
      limit(async () => {
        ...
        try {
          ...
          failedUploads.push({
            ...
            variant,
            id,
            url: `https://storage.bunnycdn.com/${storageZoneName}/${dir}/${id}.woff2`
          });
...
async function main() {
  ...
  // Retry failed uploads with lower concurrency
  if (failedUploads.length > 0) {
    console.log(`\n=== Retrying ${failedUploads.length} failed uploads ===`);
    const retryLimit = pLimit(2); // Even lower concurrency for retries

    await Promise.all(
      failedUploads.map(upload =>
        retryLimit(async () => {
          try {
            console.log(`Retrying ${upload.family}/${upload.variant.fontStyle}/${upload.id}...`);
            await pipeline(
              got.stream(upload.variant.woff2),
              await got.stream.put(upload.url, {
                headers: {
                  AccessKey: process.env.BUNNY_API_KEY,
                },
                retry: { limit: 1 }
            ...
            console.log(`✓ Successfully uploaded ${upload.family}/${upload.variant.fontStyle}/${upload.id}`);
            processedVariants++;
          } catch (err) {
            console.error(`✗ Failed again: ${upload.family}/${upload.variant.fontStyle}/${upload.id}: ${err.message}`);
          }
        })
      )
    );
