...
server.get('/css', async (request, reply) => {
  try {
    const payload = []
    let { family: familyQuery, display = null, subset = null, ver = null } = request.query
    const families = familyQuery.split('|')
    for (let family of families) {
      let style = 'normal'
      let weights = ['400']
      const name = family.split(':')[0]
      const foundWeights = family.split(':')[1].split(',').map((weight) => {
        if (weight === 'i' || weight === 'italic') {
          weight = '400i'
          return weight
        }
        if (/^[a-zA-Z]+$/.test(weight) && weight !== 'i' && weight !== 'italic' && weight !== 'b' && weight !== 'bold' && weight !== 'bi' && weight !== 'ib' && weight !== 'bolditalic' && weight !== 'italicbold') {
          return '400'
        }
        return weight
      })
